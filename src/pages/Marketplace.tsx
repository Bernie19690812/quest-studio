...
const Marketplace = () => {
  ...
  const getIndividualsByRole = (roleId: string) => {
    ...
    return categorizedData.individuals.filter(item => roleMap[roleId]?.some(role => item.role?.includes(role) || item.tags.some(tag => tag.toLowerCase().includes(role.toLowerCase()))));
  };
  ...
  if (showPurchasedItems) {
    return <div className="min-h-screen bg-background">
        ...
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
          <div className="flex items-center justify-between p-6">
            ...
            <div className="flex items-center space-x-4">
              <Button variant="outline" size="icon" onClick={() => setIsFavoritesOpen(true)} className="border-border hover:bg-accent">
                <Star size={20} />
              </Button>
              <Button variant="outline" size="icon" onClick={() => setIsFullPageCartOpen(true)} className="relative border-border hover:bg-accent">
                <ShoppingCart size={20} />
                {cartItems.length > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">
                    {cartItems.length}
                  </Badge>}
              </Button>
              <DropdownMenu>
        ...
        <main className="px-6 py-8">
          ...
          {purchasedItems.length === 0 ? <div className="text-center py-16">
            ...
            </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
              {purchasedItems.map(item => <MarketplaceCard key={item.id} item={item} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} isFavorited={isFavorited(item.id)} />)}
            </div>}
        </main>

        {/* Modals */}
        <MarketplaceItemModal item={selectedItem} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} isFavorited={selectedItem ? isFavorited(selectedItem.id) : false} />
      </div>;
  }
    ...
    return <div className="min-h-screen bg-background">
        ...
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
          <div className="flex items-center justify-between p-6">
            <div className="flex items-center space-x-6">
              ...
              <nav className="flex items-center space-x-1">
                <Button variant="ghost" onClick={() => setSelectedRoleGroup(null)} className="text-muted-foreground hover:text-foreground flex items-center space-x-2">
                  <ArrowLeft size={16} />
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <Input placeholder={`Search ${roleGroup?.name.toLowerCase()}...`} className="pl-10 w-64 bg-secondary border-border" />
              </div>
              <Button variant="outline" size="icon" onClick={() => setIsFavoritesOpen(true)} className="border-border hover:bg-accent">
                <Star size={20} />
              </Button>
              <Button variant="outline" size="icon" onClick={() => setIsCartOpen(true)} className="relative border-border hover:bg-accent">
                <ShoppingCart size={20} />
                {cartItems.length > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 text-xs bg-primary">
                    {cartItems.length}
                  </Badge>}
        ...
        <main className="px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {roleGroupData.map(item => <MarketplaceCard key={item.id} item={item} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} isFavorited={isFavorited(item.id)} />)}
          </div>
        </main>
        ...
        <CartDrawer isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} items={cartItems} allItems={mockData} onRemoveItem={removeFromCart} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} onCheckout={handleCheckout} isFavorited={isFavorited} />

        <FavoritesDrawer isOpen={isFavoritesOpen} onClose={() => setIsFavoritesOpen(false)} items={favorites} onAddToCart={addToCart} onRemoveFavorite={itemId => setFavorites(prev => prev.filter(i => i.id !== itemId))} />

        <StripeCheckoutModal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} items={cartItems} onPaymentSuccess={handlePaymentSuccess} />

        <PaymentSuccessModal isOpen={isSuccessModalOpen} onClose={() => setIsSuccessModalOpen(false)} purchasedItems={purchasedItems} onGoToStudio={handleGoToStudio} />
      </div>;
  }
  if (selectedCategory === 'individuals') {
    ...
    const filteredIndividuals = activeFilters.length === 0 ? categorizedData.individuals : categorizedData.individuals.filter(item => {
      return activeFilters.some(filter => {
        if (filter.category === 'role') {
          ...
          return roleMap[filter.id]?.some(role => item.role?.includes(role) || item.tags.some(tag => tag.toLowerCase().includes(role.toLowerCase())));
        }
        ...
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
          <div className="flex items-center justify-between p-6">
            <div className="flex items-center space-x-6">
              ...
              <nav className="flex items-center space-x-1">
                <Button variant="ghost" onClick={() => setSelectedCategory(null)} className="text-muted-foreground hover:text-foreground flex items-center space-x-2">
                  <ArrowLeft size={16} />
                  <span>Back to Marketplace</span>
                </Button>
                <Button variant="ghost" className="text-foreground font-medium">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" size={18} />
                <Input placeholder="Search individuals..." className="pl-10 w-64 bg-secondary border-border" />
              </div>
              <Button variant="outline" onClick={handleGoToMyItems} className="border-border hover:bg-accent">
                My Items
              </Button>
              <Button variant="outline" size="icon" onClick={() => setIsFavoritesOpen(true)} className="border-border hover:bg-accent">
                <Star size={20} />
        ...
        <main className="px-6 py-8">
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-2">
              ...
              return <Button key={filter.id} variant={isActive ? "default" : "outline"} size="sm" onClick={() => handleFilterToggle(filter)} className="rounded-full">
                    {filter.label}
                  </Button>;
            })}
              {activeFilters.length > 0 && <Button variant="ghost" size="sm" onClick={handleClearFilters} className="text-muted-foreground">
                  Clear all
                </Button>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {filteredIndividuals.map(item => <MarketplaceCard key={item.id} item={item} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} isFavorited={isFavorited(item.id)} />)}
          </div>
        </main>
  ...
  if (selectedCategory) {
    ...
    return <div className="min-h-screen bg-background">
        ...
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
          <div className="flex items-center justify-between p-6">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" size={18} />
                <Input placeholder={`Search ${categoryTitle.toLowerCase()}...`} className="pl-10 w-64 bg-secondary border-border" />
              </div>
              <Button variant="outline" size="icon" onClick={() => setIsFavoritesOpen(true)} className="border-border hover:bg-accent">
                <Star size={20} />
              </Button>
        ...
        <main className="px-6 py-8">
          ...
          {/* Filter Bar */}
          <FilterBar category={selectedCategory} activeFilters={activeFilters} onFilterToggle={handleFilterToggle} onClearFilters={handleClearFilters} />

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {categoryData.map(item => <MarketplaceCard key={item.id} item={item} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} isFavorited={isFavorited(item.id)} />)}
          </div>
        </main>
  ...
  return <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border">
        <div className="flex items-center justify-between p-6">
          <div className="flex items-center space-x-6">
            ...
            <nav className="flex items-center space-x-1">
              <Button variant="ghost" onClick={() => navigate('/')} className="text-muted-foreground hover:text-foreground flex items-center space-x-2">
                <ArrowLeft size={16} />
                <span>Studio</span>
              </Button>
              <Button variant="ghost" className="text-foreground font-medium">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" size={18} />
              <Input placeholder="Search marketplace..." className="pl-10 w-64 bg-secondary border-border" />
            </div>
            <Button variant="outline" onClick={handleGoToMyItems} className="border-border hover:bg-accent">
              My Items
            </Button>
      ...
      <section className="relative px-6 py-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl">
          ...
          <Button size="lg" onClick={() => navigate('/')} className="bg-primary hover:bg-primary/90 hover-scale">
            Go to quest studio
          </Button>
        </div>
      </section>
      ...
      <main className="px-6 space-y-12 pb-12">
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <Button variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={() => handleSeeAll('solutions')}>
              See all
            </Button>
          </div>
          <CategorySection items={categorizedData.solutions} category="solutions" onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} onSeeMore={handleSeeAll} isFavorited={isFavorited} />
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <Button variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={() => handleSeeAll('capabilities')}>
              See all
            </Button>
          </div>
          <CategorySection items={categorizedData.capabilities} category="capabilities" onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} onSeeMore={handleSeeAll} isFavorited={isFavorited} />
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <Button variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={() => handleSeeAll('teams')}>
              See all
            </Button>
          </div>
          <CategorySection items={categorizedData.teams} category="teams" onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} onSeeMore={handleSeeAll} isFavorited={isFavorited} />
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <Button variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={() => handleSeeAll('individuals')}>
              See all
            </Button>
          </div>
          <div className="w-full overflow-hidden">
            <ScrollArea className="w-full">
              <div className="flex space-x-4 pb-4 px-1">
                {roleGroups.map(roleGroup => <RoleGroupCard key={roleGroup.id} roleGroup={roleGroup} onSelect={rg => setSelectedRoleGroup(rg.id)} />)}
              </div>
            </ScrollArea>
          </div>
        </section>
      ...
      {/* Full Page Cart */}
      <FullPageCart isOpen={isFullPageCartOpen} onClose={() => setIsFullPageCartOpen(false)} items={cartItems} allItems={mockData} onRemoveItem={removeFromCart} onAddToCart={addToCart} onToggleFavorite={toggleFavorite} onOpenModal={handleOpenModal} onCheckout={handleCheckout} isFavorited={isFavorited} />
    </div>;
};
