...
export const MainWorkArea = ({
  ...
  return <div className="flex-1 flex flex-col" onDrop={e => {
    e.preventDefault();
    console.log('Component dropped');
      ...
      <div className="border-b border-border">
        ...
        {activeComponents.length > 0 ? <div className="px-6 pb-4">
            <div className="flex items-center gap-2">
              <ScrollArea className="flex-1">
                <div className="flex items-center gap-2 pb-2">
                  {activeComponents.map(component => {
                const IconComponent = getComponentIcon(component.type);
                return <Badge key={component.id} variant="secondary" className="flex items-center gap-2 px-3 py-2 whitespace-nowrap bg-secondary border border-border rounded-full">
                        <IconComponent size={14} />
                        <span className="text-sm">{component.name}</span>
                        <Button variant="ghost" size="icon" className="h-4 w-4 hover:bg-destructive/20 rounded-full ml-1" onClick={() => removeComponent(component.id)}>
                          <X size={12} />
                        </Button>
                      </Badge>;
              })}
              ...
              <Button variant="ghost" size="sm" onClick={clearAllComponents} className="flex items-center gap-1 text-muted-foreground hover:text-destructive rounded-full px-3">
                Clear Components
              </Button>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
            <div className="p-4 border border-border rounded-lg hover:bg-accent/50 transition-colors cursor-pointer" onClick={() => handleDrop({
            id: 'test-1',
            name: 'Field Detector',
            type: 'capability'
          })}>
            ...
            <div className="p-4 border border-border rounded-lg hover:bg-accent/50 transition-colors cursor-pointer">
              ...
              <p className="text-sm text-muted-foreground py-[10px]">Review previous conversations</p>
            </div>
            <div className="p-4 border border-border rounded-lg hover:bg-accent/50 transition-colors cursor-pointer">
