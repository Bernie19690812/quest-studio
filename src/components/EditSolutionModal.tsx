...
export const EditSolutionModal = ({
  ...
  const updateApiKey = (index: number, field: 'name' | 'value', value: string) => {
    setSettings(prev => ({
      ...prev,
      apiKeys: prev.apiKeys.map((key, i) => i === index ? {
        ...key,
        [field]: value
      } : key)
    }));
      ...
      case 'general':
        return <div className="space-y-6">
            <div>
              ...
              <Input id="solution-name" value={settings.name} onChange={e => setSettings(prev => ({
              ...prev,
              name: e.target.value
            }))} className="mt-2" />
            </div>
            ...
            <div>
              ...
              <Textarea id="description" value={settings.description} onChange={e => setSettings(prev => ({
              ...prev,
            ...
            <div>
              ...
              <RadioGroup value={settings.aiModel} onValueChange={value => setSettings(prev => ({
              ...prev,
              aiModel: value
        ...
        return <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                {settings.apiKeys.map((apiKey, index) => <div key={index} className="flex gap-3 items-start">
                    <div className="flex-1 space-y-2">
                      <Input placeholder="API Key Name (e.g., OpenAI, Anthropic)" value={apiKey.name} onChange={e => updateApiKey(index, 'name', e.target.value)} />
                      <Input type="password" placeholder="API Key Value" value={apiKey.value} onChange={e => updateApiKey(index, 'value', e.target.value)} />
                    </div>
                    <Button variant="outline" size="icon" onClick={() => removeApiKey(index)} className="mt-0">
                      <X size={16} />
                    </Button>
                  </div>)}
          ...
          <div className="w-64 bg-muted/30 border-r flex flex-col">
            ...
            <nav className="flex-1 p-4">
              <div className="space-y-1">
                {sidebarTabs.map(tab => {
                const Icon = tab.icon;
                return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-lg transition-colors ${activeTab === tab.id ? 'bg-primary text-primary-foreground' : 'hover:bg-muted text-muted-foreground hover:text-foreground'}`}>
                      <Icon size={16} />
                      {tab.label}
                    </button>;
              })}
